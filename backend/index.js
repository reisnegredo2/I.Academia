...
// Diagnóstico: lista modelos disponíveis na conta Groq
app.get('/api/models', async (req, res) => {
  try {
    if (!GROQ_API_KEY) return res.status(500).json({ error: 'GROQ_API_KEY ausente' });
    ...
    res.status(500).json({ error: 'Falha ao consultar modelos', details: String(e) });
  }
});
...
${ignoreProfileMeta ? 'PARA ESTA SOLICITAÇÃO: O foco é perder gordura. Ignore metas de hipertrofia salvas no perfil.' : ''}`
    ...
    for (const candidateModel of candidateModels) {
      ...
      return res.status(500).json({ error: "Erro na API Groq", status: response.status, body: text });
    }

    if (!apiResponse) {
      console.error("Erro API Groq após fallbacks:", lastErrorText);
      return res.status(502).json({ error: "Não foi possível usar nenhum modelo Groq", hint: "Defina GROQ_MODEL para um ID disponível em /api/models", lastError: lastErrorText });
    }
